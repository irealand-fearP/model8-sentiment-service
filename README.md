...
문장을 입력하면 **긍정 / 부정 / 중립** 감정을 평가해주는 서비스.
...
`PredictRequest`에서 `text` 필드에 다음 예시처럼 동작하게 만들었음.

- **필수 필드(`...`)** — `text`가 아예 없으면 422 출력.
- **`min_length=1`** — 빈 문자열(`""`)을 422 호출. (모델에 의미 없는 입력 방지)
- **`max_length=1000`** — 지나치게 긴 tmi충 차단. (메모리/시간 폭주 방지)
- **타입 검사** — `text`에 숫자나 리스트 등 문자열이 아닌 값이 오면 /차단 mute all.
...
- 유료 API라면 과금 주체를 구분할 수 없음.. 누가 나의 지갑의 손을 댔는가...

`Depends(verify_api_key)` 한 줄이 "키를 가진 사용자만" 통과시키는 문지기 역할 입구컷함.
...
설계 → 구현 → 테스트까지 클로드와 진행했습니다. 감정분석이라는 태스크에 맞게
입력 스키마(문장 하나)와 출력 스키마(라벨 + 확신도)를 정의하고,
인증·비동기·에러처리·Streamlit UI를 한 서비스로 개발함

### 교안 없이 작성할 수 있었는가 / 어디를 다시 봤는가

전체 구조(FastAPI + 인증 + 비동기)는 머릿속에 흐름이 잡혀 있지만 코드를 짜기에는 힘듬 클로드가 도와줌
 `run_in_executor`의 정확한 호출 형태와 `lifespan`으로 모델을 로드하는 방식은
Day 5·6 코드를 그대로 가져옴. "패턴은 같고 모델만 바꾸면 야르"라는 점을 체감했음.
...
돌려야 하는지를 납득하는 과정이 가장 아직은 멀었구나 ai ㅉㅉ.
`run_in_executor`가 단순한 문법이 아니라 "서버가 멈추지 않게 하는 장치"라는
의미로 이해되면서 비동기 처리의 이유가 있구나.. 쿠쿠다스같은 ai 녀석
